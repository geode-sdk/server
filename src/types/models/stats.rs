...
impl Stats {
    ...
    async fn get_latest_github_release_download_count(pool: &mut PgConnection) -> Result<i64, ApiError> {
        ...
        sqlx::query!("
            ...
            VALUES ($1, $2)
        ", new.0, new.1).execute(&mut *pool).await.map_err(|e| {
            log::error!("{}", e);
            ApiError::DbError
        })?;
        Ok(new.0)
    ...
    async fn fetch_github_release_stats() -> Result<(i64, String), ApiError> {
        ...
        let latest_release_tag = releases.iter()
            .find(|r| r.tag_name != "nightly")
            .ok_or(ApiError::InternalError)?
            .tag_name.clone();
        Ok((
            releases.into_iter().map(|r| r.assets.into_iter().map(|a| a.download_count).sum::<i64>()).sum(),
            latest_release_tag
        ))
    }
}
